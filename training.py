...
def evaluate_and_save(model, optimizer, scheduler, val_dataloader, cfg, 
    ...
    if best_score==score:
        save_model(model=model, optimizer=optimizer, scheduler=scheduler,
                   output_file=os.path.join(cfg['training']['model_dir'], 'ckpts', 'best.ckpt'),
                   epoch=epoch, global_step=global_step,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        default="experiments/configs/SingleStream/csl-daily_s2t.yaml",
        type=str,
    ...
    logger = make_logger(
        model_dir=model_dir,
        log_file='train.rank{}.log'.format(1))  # cfg['local_rank']
    ...
    train_dataloader, train_sampler = build_dataloader(cfg, 'train', model.text_tokenizer, model.gloss_tokenizer)
    dev_dataloader, dev_sampler = build_dataloader(cfg, 'dev', model.text_tokenizer, model.gloss_tokenizer)
    ...
    for epoch_no in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            batch['translation_inputs']['labels'] = batch['translation_inputs']['labels'].cuda()
            batch['translation_inputs']['decoder_input_ids'] = batch['translation_inputs']['decoder_input_ids'].cuda()
            batch['translation_inputs']['gloss_ids'] = batch['translation_inputs']['gloss_ids'].cuda()
            batch['recognition_inputs']['gloss_labels'] = batch['recognition_inputs']['gloss_labels'].cuda()
            batch['recognition_inputs']['head_rgb_input'] = batch['recognition_inputs']['head_rgb_input'].cuda()
            batch['recognition_inputs']['sgn_mask'] = batch['recognition_inputs']['sgn_mask'].cuda()
